...
//					scanf("%d",&i);
        ...
        for(j = 0;j<C;j++){             	
			for(k = 0;k<N;k++){				
				tg2 = 3 - (U[k][j] + T[k][j]) - (double)pow((3 - (double)pow(U[k][j] + T[k][j],ALPHA)),1/ALPHA);
				tg = 1.0 / (1 + exp(-tg2));
...
double ts3pfcm(double prepareTime, char *folder, char *fileout, int *iter){
	...
	if(isdemo){
		outPutV("demo/ts3pfcm_init.txt", 0, 0, 3);	
	}
		
	//outPutSFCM("demo/init.txt", Vla, Ula, NLA, 0);	
	printf("ts3pfcm start\n");
	// PFCM for all
	do{		
		maxU = 0;
		maxT = 0;
...
//					scanf("%d",&i);
		...
		if(step == 0 && isdemo){
			outPutV("demo/ts3pfcm_firststep.txt", 0, 0, 3);	
		}
		...
		if(step % 10 == 0) printf("TS3PFCM:    iter %d ok\n",step);	
		step++;     
        runtime += (double)(clock() - t) / CLOCKS_PER_SEC;
        t = clock();                        
    }while(isNext != 0);     
    *iter = step;
	...
	outPutV(s, step, runtime,3); 
    printf("TS3PFCM:   OutputV ok with %ld steps\n",step);
...
int main(int  argc, char **argv){	
	...
	runti = input(s1);
//	printf("input ok!\n");
	sprintf(s1,"results/TS3PFCM_result_%s_%d.csv",argv[1],PERCENT_WRONG);
	f = fopen(s1,"w");
	...
	for(i = 1;i<=n;i++){		
		...
		sprintf(s2,"ts3pfcm_%s_%d.txt",argv[1],PERCENT_WRONG);
		sprintf(s,"%s%s",s1,s2);
			
		runtime1 = ts3pfcm(runti,s1,s2,&iter);		
		runtime += runtime1;
