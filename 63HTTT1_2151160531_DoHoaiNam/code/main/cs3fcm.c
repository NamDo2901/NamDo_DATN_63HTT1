...
int main(int  argc, char **argv){
	...
	double db1,ri1,asw1,asw = 0,pbm1,pbm = 0, ifv1 = 0;
...
//	normalizeX();
//	printf("input ok!\n");
	sprintf(s1,"results/CS3FCM_result_%s_%d.csv",argv[1],PERCENT_WRONG);
	f = fopen(s1,"w");
	if(!f){
	...
	for(i = 1;i<=n;i++){		
		printf("\nTime %d :\n",i);
		sprintf(s,"results/%d/CS3FCM_%s_%d.txt",i,argv[1],PERCENT_WRONG);
